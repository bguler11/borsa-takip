...
function initializeApp() {
  ...
  function openDetailsSheet(symbol) {
    ...
    if (sheetTitle) sheetTitle.innerText = `${pos.symbol} Detayları`;
    if (sheetAvgCost) sheetAvgCost.innerText = `${pos.averageCost.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TL`;
    if (sheetNetQty) sheetNetQty.innerText = `${pos.quantity.toLocaleString('tr-TR')} Adet`;
    if (sheetCurValue) sheetCurValue.innerText = `${pos.currentValue.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} TL`;
    ...
    if (sheetBtnBuy && sheetBtnSell) {
      ...
      newBtnBuy.addEventListener('click', () => {
        closeDetailsSheet();
        transactionType = 'BUY';
        if (btnTypeBuy) btnTypeBuy.classList.add('active', 'buy');
        if (btnTypeSell) btnTypeSell.classList.remove('active', 'sell');
      ...
      newBtnSell.addEventListener('click', () => {
        closeDetailsSheet();
        transactionType = 'SELL';
        if (btnTypeSell) btnTypeSell.classList.add('active', 'sell');
        if (btnTypeBuy) btnTypeBuy.classList.remove('active', 'buy');
  ...
  setTimeout(async () => {
    try {
      ...
      if (updatedPrices) {
        ...
        const activeScreen = document.querySelector('main .screen.active');
        if (activeScreen) {
          if (activeScreen.id === 'screen-portfolio') renderPortfolio();
          if (activeScreen.id === 'screen-watchlist') renderWatchlist();
        }
...
// Ensure execution in any load context (direct or deferred event)
